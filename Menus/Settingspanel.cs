...
public partial class SettingsPanel : Panel
{
    ...
    public override void _Ready()
    {
        ...
        outer.AnchorRight = 1f;
        outer.AnchorBottom = 1f;

        outer.OffsetLeft = 24f;
        outer.OffsetRight = -24f;
        outer.OffsetTop = 20f;
        outer.OffsetBottom = -20f;
    ...
    private Control BuildSettingsTab()
    {
        ...
        if (sm == null)
        {
            GD.PrintErr("SettingsPanel: SettingsManager.Instance is null.");
            return vbox;
        }
        ...
        statsLabel.Text = "Show Player Stats";
        statsLabel.CustomMinimumSize = new Vector2(180f, 0f);

        var statsToggle = new CheckButton();
        statsToggle.ButtonPressed = sm.ShowPlayerStats;

        statsRow.AddChild(statsLabel);
        statsRow.AddChild(statsToggle);
        ...
        statsToggle.Toggled += v =>
        {
            SettingsManager.Instance?.SetShowPlayerStats(v);
        };
    ...
    private (HSlider, Label) AddSlider(
    ...
    private Control BuildKeybindsTab()
    {
        ...
        _listenHint.AddThemeColorOverride(
            "font_color",
        ...
        foreach (var (action, label) in KeybindsManager.Rebindable)
        {
            ...
            keyBtn.Text =
                KeybindsManager.Instance?.GetKeyLabel(action) ?? "—";

            keyBtn.Pressed += () =>
                BeginListening(action, keyBtn);
    ...
    private void RefreshKeyButtons()
    {
        ...
        {
            kvp.Value.Text =
                KeybindsManager.Instance?.GetKeyLabel(kvp.Key) ?? "—";
        }
    }
    ...
    {
        if (_listeningAction == null || !Visible)
            return;
